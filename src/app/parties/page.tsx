...
function PartyCard({ party, user, userMap, profile, handlePartyAction, handleUpdateParty, handleDisbandParty, handleKickMember }: {
    ...
    const isLeader = user && user.email === leader.email;
    const canModifyDetails = isAdmin || isMember;
    const canDisband = isAdmin || isLeader;
    const canKick = isAdmin || isLeader;
...
export default function PartiesPage() {
    ...
    const handleKickMember = async (partyId: string, memberEmailToKick: string, memberNickname: string) => {
        if (!user || !user.email) return;
        if (confirm(`정말로 '${memberNickname}'님을 파티에서 제외하시겠습니까?`)) {
            try {
                const res = await fetch('/api/parties', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (!res.ok) {
                    const data = await res.json();
                    throw new Error(data.error || '멤버 제외 실패');
                }
                alert('멤버가 제외되었습니다.');
                fetchData();
            } catch (error: unknown) {
