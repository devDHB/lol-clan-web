...
// 본문에서 이미지 링크를 제거하고 줄바꿈을 유지하는 헬퍼 함수
...
export default function NoticesPage() {
  ...
  const fetchData = useCallback(async () => {
    // ProtectedRoute가 user를 보장하므로, user가 null인 경우는 고려하지 않아도 됨
    if (!user) return;
  ...
  return (
    <ProtectedRoute>
      {loading ? (
        <main className="flex justify-center items-center min-h-screen bg-gray-900 text-white">공지사항을 불러오는 중...</main>
      ) : (
