...
export async function GET(request: Request) {
    try {
        ...
        // 랭킹 계산 로직
        const getTop3 = (data: any[], key: string, gameKey?: string, minGames = 0) => {
            return [...data] // 원본 배열 복사
                .filter(p => gameKey ? p[gameKey] >= minGames : true)
                .sort((a, b) => b[key] - a[key])
                .slice(0, 3)
                .map(p => ({ 닉네임: p['닉네임'], value: p[key] }));
        };

        const rankings = {
            꾸준왕: getTop3(stats, '총 경기'),
            다승: getTop3(stats, '총 승'),
            승률: getTop3(stats, '승률', '총 경기', 5), // 최소 5경기 이상
